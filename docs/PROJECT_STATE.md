...
**CURRENT FRONTIER:** repeated trace-only runs after the privacy-safe SignOn bootstrap handoff reach authenticated BAP and no-reply service 29, but not client service 10 or Queuez service 123. A metadata-only retail callback marker confirms that the task observer receives native callbacks, yet it does not observe the internal-oracle `ENUM(0)` completion diagnostic. A later metadata-only assert classifier observed patchable-bootstrap and investment-globals local load failures after service 29, neither carrying `-87`; it did not observe the registration class. The eleven-domain readiness and immediate persistence-completion probes both passed before the same external black-screen/service-10 absence. Source comparison then showed the external run does not enter the existing native character-select handler that the internal/default oracle reaches; external trace does not alter that bootflow path. A later bounded in-process scan found one image-local pointer-sized reference to that unique handler target, while direct rel32 callers remain zero. A follow-up classified the reference as read-only and found zero canonical RIP-relative indirect calls/jumps reading it; other transfer forms and the reference's semantic role remain unknown. Discovery, ContentConfig acceptance, BAP acknowledgement, task-observer installation, and service-29 no-reply behavior remain unchanged. A current-source comparator audit identified an earlier client-local differential: official Sunrise installs package-integrity handling before base-package registration, while the external trace lineage predates it. A subsequent read-only three-class compatibility classifier found all comparator targets present in the historical external binary, but the same black screen and absent service 10 remained. It establishes applicability only; no integrity behavior was ported, activated, bypassed, or changed.
...
## 2026-08-30 package-trust comparator audit

**COMPLETE / NEGATIVE FOR COMPATIBILITY-ONLY HYPOTHESIS:** Current official Sunrise installs a package-integrity component before base-package registration; the external trace lineage predates it. A Windows-CI-built, read-only three-class classifier found all comparator targets present in the historical external binary, while the same black screen and no service 10 remained. The component was not ported, activated, bypassed, or changed. See `docs/client-analysis/package-trust-comparator-audit.md` and `docs/experiments/2026-08-30-package-trust-compatibility-classifier.md`.

## Next Experiment

Do not repeat the package-trust compatibility classifier: it is complete and did not move the visible/protocol frontier. A source comparison identified one concrete post-service-29 differential: the current reference family-zero Queuez source seed falls back to the checked runtime account identity when the live manager key is not yet observable; external trace returns without seeding in that condition. A Windows-CI-built aggregate-only observer ran after full dedicated-runtime/isolation preflight. It did not emit, despite successful family-zero hook installation, which confirms the existing family-zero sweep itself did not execute in the bounded external path. The black screen and absent service 10 persisted. Follow-up source comparison found the family-zero subscription implementation identical across current reference and retained validation lineages, and the authorized source supplies no caller/predicate for the native sweep. A separately authorized, aggregate-only offline static caller scan was source-tested and run against both protected validation and dedicated external executable files. The exact runtime hook signature matched zero bytes on disk in both, so no target or callers could be resolved offline. A loaded-process scan then confirmed that the already-installed detour masks the original signature post-install. A CI-built, pre-detour aggregate observer resolved that timing limit and reported zero direct calls, one direct tail jump, and zero absolute pointer references to the family-zero sweep. This establishes a unique static tail-jump wrapper as its immediate predecessor category; no source-list, protocol, account, package, or integrity behavior changed. Do not add a service-29 reply, Queuez service 123, package content, account state, integrity override, or speculative response.

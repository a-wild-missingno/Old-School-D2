...
Classify the one bounded character-sign-in image reference by mapped PE section and any image-local executable indirect branch that reads it. Identify whether it creates one source-backed predecessor probe; do not add a service-29 reply, Queuez service 123, package content, account state, or speculative response.

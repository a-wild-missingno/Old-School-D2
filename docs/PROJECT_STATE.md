...
**COMPLETE / SOURCE-ONLY:** Current official Sunrise installs a package-integrity component before base-package registration; the external trace lineage predates that component. It is a concrete client-local comparator differential upstream of the missing character-signin entry, but the component changes integrity acceptance and is not a metadata-only probe. No client or server behavior changed. See `docs/client-analysis/package-trust-comparator-audit.md`.

## Next Experiment

Build and source-test only a read-only compatibility classifier for the three current-comparator target classes. It must emit aggregate booleans only and make no detour, memory/code write, integrity override, package/content change, or listener/protocol change. Do not launch until it has a single unambiguous compatibility criterion and a verified Windows CI artifact. Do not add a service-29 reply, Queuez service 123, package content, account state, or speculative response.

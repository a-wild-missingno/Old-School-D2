...
if (\$procs.Count -eq 0) { Write-Output 'DESTINY_RUNNING=false'; Write-Output 'DESTINY_PIDS=NONE'; Write-Output 'RUNTIME=unknown' } else { Write-Output 'DESTINY_RUNNING=true'; Write-Output ('DESTINY_PIDS=' + ((\$procs | ForEach-Object { \$_.ProcessId }) -join ',')); foreach(\$p in \$procs) { \$runtime='unknown'; \$path=\$p.ExecutablePath; if (\$path) { if (\$path.StartsWith($(ps_quote "${ORACLE_RUNTIME:-__unconfigured__}"),[StringComparison]::OrdinalIgnoreCase)) {\$runtime='oracle'} elseif (\$path.StartsWith($(ps_quote "${EXTERNAL_VALIDATION_RUNTIME:-__unconfigured__}"),[StringComparison]::OrdinalIgnoreCase)) {\$runtime='external-validation'} elseif (\$path.StartsWith($(ps_quote "${EXTERNAL_TRACE_RUNTIME:-__unconfigured__}"),[StringComparison]::OrdinalIgnoreCase)) {\$runtime='external-trace'} }; Write-Output ('RUNTIME=' + \$runtime); Write-Output ('DESTINY_PROCESS=pid:' + \$p.ProcessId + ';runtime:' + \$runtime + ';path:' + \$path) } };\
